...
NGram::NGram(){
	this->objectProbabilityUtils = new ProbabilityUtils();
}

unsigned int NGram::whereIsMissingTheWord(vector<string> vectorOfTheLine){
	string searchNgram;
	unsigned int whereToAdd;
	double minorProbability = std::numeric_limits<double>::max();	//le asigno el valor maximo de un int

	for ( unsigned int counter = 1; counter < (vectorOfTheLine.size() -1); counter++) {
		cout << "me meti" << endl;
		//obtengo el peso
		if (this->objectProbabilityUtils->getWordProbability(vectorOfTheLine, counter) < minorProbability){
			whereToAdd = counter;
		}
	}
